...
hyperparam='h-'+str(args.num_heads)+'-eps-'+str(args.eps)+'-m-'+str(args.num_noise_vec)+'-lbdlast-'+str(args.lbdlast)+'-lbd-'+str(args.lbd)
...
def train_epoch(net, trainloader, optimizer, epoch):
    ...
    losses, losses_ortho, losses_con = AverageMeter(), AverageMeter(), AverageMeter()
    losses_ce = []
    for idx in range(args.num_heads):
    ...
    for batch_idx, batch in enumerate(trainloader):
        ...
        for b_data, b_label in mini_batches:
            ...
            # loss_ce, all_losses, coeffs_head = ct_loss(all_logits_chunk, b_label, args.eps, threshold)
            loss_ce, all_losses, loss_con = ct_con_loss(all_logits_chunk, b_label, args.eps, threshold, args.lbd)
            for idx in range(args.num_heads):
            ...
            # loss_con = consistency_loss(all_logits_chunk, coeffs_head, args.lbd, args.eta)

            # -------- SUM the two losses
            total_loss = loss_ce + args.alpha * loss_ortho + loss_con
            ...
            losses_con.update(loss_con, b_data.size(0))
        # ----
    ...
    print('     CONSIST loss = %f.'%losses_con.avg)
